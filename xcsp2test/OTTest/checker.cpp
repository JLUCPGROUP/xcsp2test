		...
		for (int64 i = 0; i < size_; ++i) {
			...
			if (!var->Bound()) {
				...
				if (deg[i] == 0)
					...
					dom_deg = var->Size() / deg[i];
				if (dom_deg < min_domain_size) {
					selected_var = var;
					id = i;
...
int main(const int argc, char ** argv) {
...
#ifdef LOGFILE
	ofstream lofi;
	const string bm_res = bmp_root + "res\\ot\\" + argv[1] + ".txt";
	...
	lofi << "---------------summary---------------" << endl;
	lofi << "file :" << argv[1] << endl;
	lofi << "avg time = " << total_time / files.size() << " s |" << total_time / files.size() / 1000 << " ms." << endl;
	lofi << "avg node = " << total_node / files.size() << "|" << total_node / files.size() / 1000000 << "M." << endl;
	lofi << "time out = " << count_if(search_times.begin(), search_times.end(), [](int x) {return x > time_limit; }) << endl;
	lofi << "---------------end---------------" << endl;
	lofi.close();
#endif
